...
- **Download** up to 20,000 plots from the plot table using the download button to the upper right of the table. In order for the button to be enabled you'll need to filter the table down to less than 20,000 rows using a filter and or search term. The downloaded file will contain various csv representations of the plot observations in the table, but to download the most complete version of the data, you should interact with VegBank via the [vegbankr](https://github.com/NCEAS/vegbankr) R package.
- **Sorting** is supported for any table column that has arrows in its header. You can click that header to sort by that column in ascending or descending order, and you can hold shift and click multiple column headers to sort by multiple columns.
