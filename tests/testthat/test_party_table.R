...
test_that("process_party_data formats normalized rows", {
  ...
  expect_equal(result$Party, c("Norm Aaseng", "Unspecified"))
  expect_equal(result$Organization, c("MN DNR", "Unspecified"))
  expect_true(grepl(">191</a>", result$Contributions[1]))
  expect_true(grepl("obs-count-link", result$Contributions[1]))
  expect_equal(result$Contributions[2], "0")
  expect_equal(result$Contact, c("Email@example.com", "Unspecified"))
})
