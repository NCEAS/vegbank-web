...
test_that("process_project_data formats normalized data", {
  ...
  expect_true(grepl("Unspecified", result$Description[1]) && grepl('data-value="pj.1"', result$Description[1]))
  expect_true(grepl("Second", result$Description[2]) && grepl('data-value="pj.2"', result$Description[2]))
  expect_true(grepl("Angelina National Forest", result$Description[3]) && grepl('data-value="pj.314"', result$Description[3]))
  ...
  expect_equal(result$Started, c("Unspecified", "1997-08-21", "2022-01-01"))
  expect_equal(result$Ended, c("2020-12-31", "Unspecified", "1997-08-23"))
  expect_equal(result$`Last Plot Added`, c("2021-05-01", "2022-02-02", "Unspecified"))
})
