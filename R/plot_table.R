  ...
  process_table_data <- function(table_data) {
    withProgress(
      ...
      value = 0,
      {
        ...
        incProgress(0.4, "Cleaning author observation codes")
        author_codes <- if (!is.null(table_data$authorobscode)) {
          table_data$authorobscode
        } else {
          rep("Unknown", n_rows)
        }
        incProgress(0.4, "Cleaning location data")
        ...
        incProgress(0.6, "Creating action buttons")
        actions <- mapply(build_action_buttons, seq_len(n_rows))

        incProgress(0.8, "Compiling data for display")
